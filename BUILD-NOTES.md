...
- **Done:** 10 main pages + 16 core service pages + **16 fence-type pages** + **11 property/animal pages** + **7 materials pages** + **22 city / service-area pages** (full unique copy). **No ComingSoon shells remain** — the whole site is built out.
- **Next:** nothing outstanding in the build plan. All page batches are full pages.
...
`npm run build` clean (→ 84 pages — shells were already counted, so the total is unchanged); dev screencheck one property (e.g. horse or pool) + one materials page; strip iframes before screenshotting; confirm new layout, flat-tile spec bar (its own band, mobile 2×2), 3 page JSON-LD blocks, correct images, internal links. Redirects already exist for any old URLs.

---

## 12. BUILD GUIDE — Service Area / City pages (22) — **DONE**

**Status: COMPLETE.** All 22 `/service-areas/<city>-ks` pages are now full local landing pages. Built with a NEW lightweight `CityPageConfig` type (in `src/types/index.ts`), per-city configs in `src/config/cities/<slug>.ts`, aggregated in `src/config/cities-content.ts` (`cityConfigs` + `cityConfigMap`), rendered by the rewritten `src/pages/service-areas/[slug].astro`. Build = 84 pages, clean. Each page: unique locally-researched copy + diversified H1s (no two share a template) + **fully unique section <h2> headings** (the six section headings live in the config as `introHeading`/`contextHeading`/`processHeading`/`reviewsHeading`/`mapHeading`/`faqHeading` — NOT generated from a `{City}` template in the route — plus a varied `featuredServices.heading`; verified 0 cross-page duplicate headings across all 22), 1 hero + 2 content images (no galleries), a **per-city Google map** (added a `query` prop to `src/components/ui/MapEmbed.astro` → keyless `https://www.google.com/maps?q=<city>, KS&output=embed`; no API key needed), heavy internal linking (6 clickable `UseCaseGrid` service cards + nearby-town pills + related pills + cost guide/contact/hub), and 3 page JSON-LD blocks (`generateCityServiceSchema(city)` Service with city `areaServed`+geo, Breadcrumb, FAQ). Route layout: PageHero → intro SplitSection(`introImageKey`) → UseCaseGrid(featuredServices) → local-context SplitSection(`contextImageKey`) → ProcessGrid → ReviewsGrid → per-city MapEmbed → nearby/related pills → FAQ → CTA → QuoteSection. Configs generated by a Workflow of 21 per-city research agents (one WebSearch each, 2 waves) reading the hand-written gold standard `src/config/cities/yoder-ks.ts`. Image fixes applied post-gen: stock-47 (razor wire) and stock-60 (urban metal slat) were swapped out of the farm-town pages (pretty-prairie→65, partridge/plevna→55). Haven is the special "home base" page. The notes below are the playbook that was followed.

Turn the 22 `/service-areas/<city>-ks` ComingSoon shells into full local landing pages. **These are LOCATION pages, not product pages — do NOT reuse `FenceTypeConfig`.** They need their own lightweight config + a route that assembles existing components. This is the last batch of shells.

### 12.1 ⚠️ The #1 risk: doorway / thin-content pages
Google actively penalizes (and often won't index) sets of near-identical "city + service" pages where only the town name is swapped. **Every city page must carry genuinely unique, locally-grounded copy** or the whole batch can drag the site down. Non-negotiables:
- **Real specifics per town:** county, distance + direction from the Haven HQ, the town's character (rural ranch town vs. Wichita suburb vs. county seat), a landmark/road/known fact or two, and **which fencing that town actually needs** (acreage/ranch/barbed in small rural towns; privacy/residential in suburbs; commercial/security in Wichita & Hutchinson).
- **Vary structure & emphasis by town**, not just nouns: a Yoder page leans Amish-country ranch/farm; a Wichita page leans residential + commercial; Hutchinson (Reno County seat, State Fair) leans broad; tiny towns (Plevna, Partridge, Stafford) lean acreage/farm + "we drive out here."
- Unique intro, unique "why local," town-specific FAQs (with the city name), and local conditions (Kansas wind, soil, frost-line). If research turns up nothing distinct for a micro-town, still differentiate via county/neighbors/driving context — never ship a pure template.

### 12.2 What already exists (reuse it)
- **Data:** `catalog.ts` `cities` (22, each `{slug, name, county}`) drives nav/footer/sitemap/route — keep it. `business.ts` has `serviceAreas`, `serviceAreaCities`, Haven coords `{lat:37.8997, lng:-97.7817}`, phone, hours, 5.0★/17.
- **Current route** `src/pages/service-areas/[slug].astro` already: builds paths from `cities`, has a hero-image rotation `[13,73,1,11,17,33,23]`, computes **nearby towns by county**, and a related-links list. Rewrite it to drop `ComingSoon` and render the full layout from the new config.
- **Index** `src/pages/service-areas/index.astro` groups cities by county + map — leave as-is (already a real page).
- **Components to assemble** (all exist, no new ones needed): `PageHero`, `SplitSection` (local intro w/ image), `ServiceGrid` (services offered in town), `ProcessGrid` (how we work), `ReviewsGrid`/`TestimonialSection` (local proof), `StatsRow`, `ServiceAreasSection` (nearby towns), `MapEmbed` (`src/components/ui/MapEmbed.astro`), `FAQSection`, `CTASection`, `QuoteSection`, `TopicCardGrid`/`BenefitGrid`.

### 12.3 Recipe
1. **New type** `CityPageConfig` in `src/types/index.ts` — suggested fields: `slug, name, county, metaTitle, h1, heroImageKey?, lat, lng, distanceFromHaven (e.g. "18 mi NE of Haven"), intro: string[2-3], localContext: string[1-2] (landmarks/character/what they fence), featuredServices: string[] (subset of core-service slugs to emphasize), whyLocal: string[1-2], stats?, localFaqs: ServiceFAQ[4-6], nearbyTowns?: RelatedLink[] (else auto by county), related: RelatedLink[]`.
2. **Configs** `src/config/cities/<slug>.ts` ×22 → `export default` a `CityPageConfig`; aggregate in `src/config/cities-content.ts` (`cityConfigs` + map). (Name the aggregator `cities-content.ts` to avoid clashing with `catalog.ts cities`.)
3. **Rewrite** `service-areas/[slug].astro` to look up the config by slug and assemble: PageHero → local-intro SplitSection → ServiceGrid (featured services) → ProcessGrid → ReviewsGrid → local FAQ → ServiceAreasSection (nearby towns) → MapEmbed → CTA → QuoteSection. Keep breadcrumb Home › Service Areas (`/service-areas`) › {name}, KS.
4. **Schema:** add `generateCityServiceSchema(city)` in `structured-data.ts` — a `Service`/`LocalBusiness` with `provider` = the global business `@id`, `areaServed` = `{ "@type":"City", name:"<name>, KS" }`, and per-city `geo {lat,lng}`; plus `generateBreadcrumbSchema` + `generateFAQSchema(localFaqs)`. So 3 page JSON-LD blocks like the other page types. Per-city lat/lng must be populated (agents can pull approx coords during research, or add a coords map).
5. **Generate** with a Workflow of `general-purpose` agents (one per city). Each: WebSearch the town (county seat? population? character? landmark/road? what it's known for?), then write a unique `CityPageConfig`. Pass each agent: name, county, slug, distance/direction from Haven, its same-county neighbors, and the service emphasis for that town's character. **Rate-limit caution:** 22 parallel WebSearches may throttle — tell agents ONE search max, or split into 2 waves.

### 12.4 Legacy URLs (preserve equity)
10 cities have 301s in `public/_redirects` from the OLD site (`/<city>-ks` → `/service-areas/<city>-ks`): **arlington, buhler, hutchinson, nickerson, partridge, plevna, pretty-prairie, south-hutchinson, wichita, yoder**. The old site had real pages for these — **check archived/old content (Wayback or the client's old site) for their H1s/copy and preserve the relevance/intent** (same approach used on the legacy service pages). The other 12 towns are net-new.

### 12.5 Images & internal linking
- **No city-specific photos exist** — reuse the verified generic pool (see §11.4 + §5). Pick by town character: ranch/farm towns → 11,13,55,65,66,67,68; suburbs/residential → 7,8,17,33,35; Wichita/Hutchinson commercial → 9,10,19,20. Vary the hero per town (the rotation already does this). Keep it light — 1 hero + maybe 1 intro image; **no galleries**.
- **Interlink heavily:** each city → the core services that fit it, its same-county neighbor towns, the `/service-areas` hub, `/fence-cost-guide`, `/contact`. Nearby-town cross-links are strong local-SEO signals (the route already computes same-county neighbors).

### 12.6 County groupings (for nearby-town logic)
Reno: Haven, Hutchinson, South Hutchinson, Buhler, Nickerson, Yoder, Pretty Prairie, Partridge, Arlington, Plevna · Sedgwick: Mount Hope, Maize, Wichita · Harvey: Burrton, Halstead, Newton · Rice: Sterling, Lyons · Kingman: Kingman · McPherson: McPherson · Pratt: Pratt · Stafford: Stafford. (Single-town counties have no same-county neighbor — fall back to nearest towns regardless of county.)

### 12.7 Verify
`npm run build` clean (stays **84 pages** — cities already counted as shells). Dev screencheck **3 contrasting towns**: a rural ranch town (Yoder or Pretty Prairie), the county seat (Hutchinson), and the metro (Wichita). Confirm each reads genuinely different (not templated), 3 page JSON-LD blocks with the right city `areaServed`/geo, nearby-town links resolve, map renders, NAP consistent (Haven HQ, (620) 899-5595, daily 8–6 — **no street address**, it's a service-area business). Strip the Map + GHL iframes before screenshotting.
